...
int main(int argc, char** argv)
{
    ...
    printf("befor create img\n");
    /* create image */
    img = cvCreateImage(cvSize(width, height), IPL_DEPTH_8U, 1);
    cvZero(img);
    printf("befor Stream Client\n");
...
printf("before hack\n");
    ...
    /* create socket */
    if ((sock = socket(PF_INET, SOCK_STREAM, 0)) < 0){//INVALID_SOCKET) {
    ...
    {
        /* get raw data */
       // printf("image size %d",imgsize);
        for (i = 0; i < imgsize; i += bytes) {
            if ((bytes = recv(sock, sockdata + i, imgsize - i, 0)) == -1) {
                quit("recv failed", 1);
            }
       // do{
      //     printf("imgsize before: %d\n",imgsize);
       //    bytes = recv(sock,sockdata,imgsize,0);
       //    printf("Bytes %d\n",bytes);
       //}
       // while(bytes < 640*480);
       // bytes=0;
